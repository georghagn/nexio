...
`nexlog` intentionally does **not** provide:
...
`nexlog` writes log entries to an `io.Writer`.
...
For external log rotation strategies (e.g. via scheduler or signals), `nexlog` provides a `ReopenableWriter`.
...
- `cmd/nexlog-example/main.go` – Individuell Configuration
