...
`nexlog` stellt bewusst **keine** Funktionen bereit für:
...
`nexlog` schreibt Logeinträge auf ein `io.Writer`.
...
Für externe Rotationsstrategien stellt `nexlog` einen `ReopenableWriter` bereit.
...
- `cmd/nexlog-example/main.go` – Individuelle Konfiguration
...
`nexlog` folgt einer klaren Regel:
